...
6570 IC = IC + 1: IF LOC = 30 AND ARG = 5 THEN D$ = "7AHRPUN AOL WPJABYL YLCLHSZ H MBZLIVE6": GOSUB 11000: ILOC(ARG) = 0: ILOC(IMMOFF+7) = 30: GOTO 2500
...
D$ = "8GWC KIV'B LW BPIB9"	=> "YOU CAN'T DO THAT"
D$ = "6ZGEY NUAYK GJBKTZAXK5"=>"TAYS HOUSE ADVENTURE"

-------------------------------------------------------------------------------------------------------------------------------
D$ = "8GWC KIV'B LW BPIB9"
D$ = "6ZGEY NUAYK GJBKTZAXK5"
CS = 0
OFF = ASC(MID$(D$,1,1))-48
FOR DC = 2 to LEN(D$)-1
	D2AS = ASC(MID$(D$,DC,1))
	IF D2AS >= 65 AND D2AS <= 90 THEN D2AS=D2AS - OFF:IF D2AS < 65 THEN D2AS = D2AS+26
	D2$ = D2$ + CHR$(D2AS)
	IF D2AS >= 65 AND D2AS <= 90 THEN CS = CS + D2AS:IF CS > 9 THEN CS = CS-((INT(CS/10)) * 10)
NEXT
IF D$ <> "" THEN IF ASC(MID$(D$, LEN(D$), 1))-48 <> CS THEN PRINT "BAD CHECKSUM FOR "; D$; ", FOUND "; CS : END
IF SP THEN PRINT ".";
