...
def console(str1 = "", str2 = ""):
    result = "\t" + str(str1)
    if str2:
        result = result.ljust(40,".") +"\t"+ str(str2)
    print(Fore.CYAN + result + Style.RESET_ALL)
    return

class Game:
    def __init__(self):
        self.RM_player_location = 32 # 57 orig , 0-63
        self.MS_player_msg = ""
        self.QS_user_input = ""
        self.VS_verb_input = ""
        self.WS_word_input = "OK"
        self.VB_verb_number = -1
        self.LL_candle = 60
    ...
    def show_objects(self):
        for i in range(self.G_n_get_objects):
            if self.L_location_obj[i] == self.RM_player_location \
                    and self.F_flag_obj[i] == 0:
                print("> here you can see : ", self.OS_obj_list[i])
    ...
    def read_input(self):
        ...
        console(" 1 read_input VS_verb_input", self.VS_verb_input)

        if self.VS_verb_input in self.VS_verb_list:
            self.VB_verb_number = self.VS_verb_list.index(self.VS_verb_input)
            console(" 2 read_input VB_verb_number", self.VB_verb_number)

        console(" 3 read_input WS_word_input", self.WS_word_input)

        if self.WS_word_input in self.OS_obj_list:
            self.OB_object = self.OS_obj_list.index(self.WS_word_input)
            console(" 4 read_input OB_object ix", self.OB_object)

        if not self.WS_word_input and self.OB_object == -1:
        ...
        if self.VB_verb_number > self.V_n_verbs and self.OB_object == -1:
    ...
    def perform_action(self):
        console(" 5 perform_action VB_verb_number >", self.VB_verb_number)
        if self.VB_verb_number == 0:
            self.show_help()
    ...
    def action_move(self):
        ...
        if self.F_flag_obj[13] == 1:
            self.MS_player_msg = "CRASH! YOU FELL OUT OF THE TREE!"
            self.F_flag_obj[13] = 0
            return

        if self.F_flag_obj[26] == 1 and self.RM_player_location == 52:
            self.MS_player_msg = "GHOSTS WILL NOT LET YOU MOVE"
            return

        if self.RM_player_location == 45 and self.C_carrying_obj[0] == 1 \
            and self.F_flag_obj[33] == 0:
            self.MS_player_msg = "A MAGICAL BARRIER TO THE WEST"
            return

        if (self.RM_player_location == 26 and self.F_flag_obj[0] == 0) \
            and (self.D_move_direction == 1 or self.D_move_direction == 4):
        ...
        if self.C_carrying_obj[14] == 1 and not (
                self.RM_player_location == 53 \
                or self.RM_player_location == 54 \
                or self.RM_player_location == 55 \
                or self.RM_player_location == 47):
    ...
    #  REM READ
    def read_1140(self):
        if self.RM_player_location == 42 and self.OB_object == 32:
            self.MS_player_msg = "THEY ARE DEMONIC WORKS"

        # spells
        if (self.OB_object == 3 or self.OB_object == 35) \
                and self.C_carrying_obj[2] == 1 and self.F_flag_obj[34] == 0:
            self.MS_player_msg = "USE THIS WORD WITH CARE 'XZANFAR'"

        # scroll
        if self.C_carrying_obj[4] == 1 and self.OB_object == 4:
    ...
    def say_1180(self):
        ...
        # XZANFAR
        if self.C_carrying_obj[2] == 1 and self.OB_object == 33:
            self.MS_player_msg = "*MAGIC OCCUR*"
            if self.RM_player_location != 45:
                rnd_num = rnd(63)
                self.RM_player_location = rnd_num
        # wall
        if self.C_carrying_obj[2] == 1 and self.OB_object == 33 and self.RM_player_location == 45:
    ...
    def save_game_bin(self):
        game_state = [self.RM_player_location, self.L_location_obj, self.C_carrying_obj, self.F_flag_obj]
    ...
    def load_game_bin(self):
        ...
        with open('savedgame.bin', 'rb') as filehandle:
            self.RM_player_location, self.L_location_obj, self.C_carrying_obj, self.F_flag_obj = pickle.load(filehandle)
    ...
    def load_game(self):
        ...
        self.RM_player_location = data[0]
        self.L_location_obj = data[1]
        self.C_carrying_obj = data[2]
        self.F_flag_obj = data[3]
