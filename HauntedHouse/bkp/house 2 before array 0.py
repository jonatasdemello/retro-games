...
class Game:
    ...
    def debug(self):
        console(" ------------------- debug -------------------")
        console(" v_RM: ", self.v_RM)
        console(" v_D: ", self.v_D)
        # console(" v_MS: ", self.v_MS)
        # console(" v_QSi: ", self.v_QSi)
        # console(" v_VSi: ", self.v_VSi)
        # console(" v_WSi: ", self.v_WSi)
        # console(" v_VB: ", self.v_VB)
        # console(" v_LL: ", self.v_LL)
        # console(" v_OB: ", self.v_OB)
        # console(" v_V: ", self.v_V)
        # console(" v_G: ", self.v_G)
        # console(" v_W: ", self.v_W)

        consoleList(" v_L: ", self.v_L)
        consoleList(" v_C: ", self.v_C)
        consoleList(" v_F: ", self.v_F)
        console(" ------------------- debug -------------------")
    ...
    def read_input(self):
        ...
        self.trace(" 1 read_input v_VSi", self.v_VSi)

        if self.v_VSi in self.v_VS:
            self.v_VB = self.v_VS.index(self.v_VSi)
            console(" 2 read_input v_VB", self.v_VB)

        self.trace(" 3 read_input v_WSi", self.v_WSi)

        if self.v_WSi in self.v_OS:
            self.v_OB = self.v_OS.index(self.v_WSi)
        self.trace(" 4 read_input v_OB", self.v_OB)

        if not self.v_WSi and self.v_OB == -1:
            self.v_MS = "THAT'S SILLY"
        ...
        if self.v_VB > self.v_V and self.v_OB == -1:
    ...
    def action_move(self):
        ...
        # N
        if self.v_OB == 18 or self.v_VB == 3:
            self.v_D = 1
        # S
        if self.v_OB == 19 or self.v_VB == 4:
            self.v_D = 2
        # W
        if self.v_OB == 20 or self.v_VB == 5:
            self.v_D = 3
        # E
        if self.v_OB == 21 or self.v_VB == 6:
            self.v_D = 4
        # U
        if self.v_OB == 22 or self.v_VB == 7:
            self.v_D = 5
        # D
        if self.v_OB == 23 or self.v_VB == 8:
            self.v_D = 6
    ...
    #  REM GET, TAKE
    def get_980(self):
        # TODO: review if we need to use -1
        # can get only 0-17
        if self.v_OB > (self.v_G - 1):
            self.v_MS = "I CAN'T GET " + self.v_WSi
            return

        if self.v_L[self.v_OB-1] != self.v_RM:
            self.v_MS = "THERE IS NO " + self.v_WSi + " HERE"

        if self.v_F[self.v_OB-1] != 0: # add -1
            self.v_MS = "WHAT " + self.v_WSi + "?"

        if self.v_C[self.v_OB-1] == 1:
    ...
    def open_1030(self):
        ...
        # 24 door
        if self.v_RM == 28 and self.v_OB == 24:
            self.v_MS = "IT'S LOCKED"

        # 31 coffin
        if self.v_RM == 38 and self.v_OB == 31:
    ...
    def examine_1070(self):
        ...
        # drawer, desk
        if self.v_OB == 27 or self.v_OB == 28:
            self.v_MS = "THERE IS A DRAWER"

        # book, scrool
        if self.v_OB == 32 or self.v_OB == 4:
            self.read_1140()

        # wall
        if self.v_OB == 34 and self.v_RM == 43:
    ...
    def read_1140(self):
        # books
        if self.v_RM == 42 and self.v_OB == 32:
            self.v_MS = "THEY ARE DEMONIC WORKS"

        # spells
        if (self.v_OB == 2 or self.v_OB == 35) \
                and self.v_C[2] == 1 and self.v_F[34] == 0:
            self.v_MS = "USE THIS WORD WITH CARE 'XZANFAR'"

        # scroll
        if self.v_C[4] == 1 and self.v_OB == 4:
    ...
    def say_1180(self):
        ...
        # XZANFAR
        if self.v_C[2] == 1 and self.v_OB == 33:
            self.v_MS = "* MAGIC OCCUR *"
            if self.v_RM != 45:
                rnd_num = rnd(63)
                self.v_RM = rnd_num
        # XZANFAR
        if self.v_C[2] == 1 and self.v_OB == 33 and self.v_RM == 45:
            self.v_F[34] = 1
        return
    ...
    def dig_1220(self):
        ...
        # shovel
        if self.v_C[11] == 1 and self.v_RM == 30:
    ...
    def swing_1250(self):
        # rope
        if self.v_C[13] != 1 and self.v_RM == 7:
        ...
        # axe
        if self.v_OB == 12 and self.v_C[12] == 1:
            self.v_MS = "WHOOSH!"

        # axe
        if self.v_OB == 12 and self.v_C[12] == 1 and self.v_RM == 43:
    ...
    def climb_1300(self):
        ...
        if self.v_OB == 13 and self.v_C[13] != 1 \
                and self.v_RM == 7 and self.v_F[14] == 0:
            self.v_MS = "YOU SEE THICK FOREST and CLIFF SOUTH"
            self.v_F[14] = 1
            return

        if self.v_OB == 13 and self.v_C[13] != 1 \
        and self.v_RM == 7 and self.v_F[14] == 1:
    ...
    def light_1340(self):
        ...
        if self.v_OB == 16 and self.v_C[16] == 1 and self.v_C[7] == 0:
            self.v_MS = "IT WILL BURN YOUR HANDS"

        if self.v_OB == 16 and self.v_C[16] == 1 and self.v_C[8] == 0:
            self.v_MS = "NOTHING TO LIGHT IT WITH"

        if self.v_OB == 16 and self.v_C[16] == 1 \
                and self.v_C[8] == 1 and self.v_C[7] == 1:
    ...
    def spray_1400(self):
        # 15 aerosol
        if self.v_OB == 25 and self.v_C[15] == 1:
            self.v_MS = "HISSSS"

        if self.v_OB == 25 and self.v_C[15] == 1 and self.v_F[26] == 1:
            self.v_F[26] = 0
    ...
    def use_1430(self):
        ...
        if self.v_OB == 9 and self.v_C[9] == 1 and self.v_C[10] == 1:
    ...
    def unlock_1460(self):
        if self.v_RM == 43 and (self.v_OB == 26 or self.v_OB == 27):
            self.use_1430()
        if self.v_RM == 28 and self.v_OB == 24 and self.v_F[25] == 0 and self.v_C[17] == 1:
            self.v_F[25] = 1
    ...
    def score_1510(self):
        ...
        for i in range(self.v_G):
            if self.v_C[i] == 1:
                S = S + 1
        if S == 17 and self.v_C[15] != 1 and self.v_RM != 157:
