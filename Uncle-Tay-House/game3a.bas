...
709 REM -- READ DATA --
710 FOR I = 1 TO LWRD: READ D$: D2$ = D$: VOCAB$(I) = D2$: NEXT
750 FOR I = 1 TO 4: READ D$: D2$ = D$: NULLW$(I) = D2$: NEXT
780 FOR I = 1 TO LASTITEM: READ D$: D2$ = D$: IDESC$(I) = D2$: NEXT
800 FOR I = 1 TO 31: READ D$: D2$ = D$: RNAME$(I) = D2$
810    FOR J = 1 TO 6: READ REXIT(I,J): NEXT 
820    READ D$: D2$ = D$: RDESC$(I) = D2$
825 NEXT
...
1050 FOR I = 1 TO NXDESC: READ EXLOC(I,1): READ EXLOC(I,2): READ D$: D2$ = D$: EXDESC$(I) = D2$: NEXT
...
2500 IF LCL = 30 THEN D$= "... AND SPRING BACK": PRINT D2$: LCL = 12
...
2569 NEXT
2570 IF LCL = 17 AND REXIT(17,1) > 0 THEN D$="YOUR UNCLE'S DOBERMAN IS SNORING PEACEFULLY": PRINT D2$
2580 IF LCL = 3 AND ILOC(6) = -12 THEN D$="A BUNGEE CORD DANGLES FROM THE RAILING ABOVE": PRINT D2$
2582 IF LCL = 12 AND ILOC(6) = -12 THEN D$="A BUNGEE CORD DANGLES FROM THE RAILING": PRINT D2$
2600 FOR I = 1 TO LASTITEM: IF ILOC(I) = LCL THEN PRINT "THERE IS A "; VOCAB$(I + ITEMOFF); " HERE"
2619 NEXT
2620 IF LCL = 2 AND ILOC(3) = -1 THEN D$= "SOMETHING IS BARELY VISIBLE UNDER THE FRIDGE": PRINT D2$
2650 IF LCL = 3 AND ILOC(5) = 30 THEN D$= "THERE IS A PICTURE HIGH UP ON THE WALL": PRINT D2$

4979 REM --- DEAL WITH MID-AIR
4980 IF LCL = 30 THEN TURN1 = 1

4999 REM --- READ INPUT ---
5000 IF TURN1 <> 1 AND LCL = 30 THEN D$= "...AND SPRING BACK": PRINT D2$: LCL = 12: GOTO 2500
...
6049 D$ = "YOU CAN'T DO THAT": PRINT D2$: GOTO 5000
...
6099 D$ = "HUH?": PRINT D2$: GOTO 5000
6100 D$ = "YOU ARE CARRYING:": PRINT D2$
...
6299 REM -- JUMP --
6300 IF LCL <> 12 THEN D$= "WHO ARE YOU, DAVID LEE ROTH?": PRINT D2$: GOTO 5000
6320 IF ILOC(6) <> -12 THEN D$ = "YOU FORGOT YOUR PARACHUTE": PRINT D2$: GOTO 5000
6330 D$= "YOU BUNGEE OFF THE BALCONY...": PRINT D2$: LCL = 30: GOTO 2510

6350 GOSUB 15000: GOTO 5000

6400 ARG = INPTK(2) - ITEMOFF: IF ARG < 1 or ARG > LASTITEM THEN D$ = "HUH?": PRINT D2$: GOTO 5000
6430 IF COMM > 17 AND COMM <= 27 THEN ON COMM-17 GOTO 6500, 6600, 6700, 6700, 6700, 6800, 6900, 7600, 6950, 8200
6499 D$ = "HUH?": PRINT D2$: GOTO 5000

6500 IF ILOC(ARG) = 0 THEN D$ =  "YOU ALREADY HAVE IT": PRINT D2$: GOTO 5000
6510 IF ILOC(ARG) = 30 AND LCL = 3 AND ARG = 5 THEN D$= "IT'S TOO HIGH": PRINT D2$: GOTO 5000
6520 IF ILOC(ARG) <> LCL THEN D$ = "IT'S NOT HERE": PRINT D2$: GOTO 5000

6530 IF IC >= 8 THEN D$ = "YOU'RE CARRYING TOO MUCH": PRINT D2$: GOTO 5000
6540 IF ARG > IMMOFF THEN D$ = "IT'S TOO HEAVY": PRINT D2$: GOTO 5000
6559 IF LCL = 29 AND ARG = 12 THEN D$ = "YOU CAN'T DO THAT": PRINT D2$: GOTO 5000
6570 IC = IC + 1: IF LCL = 30 AND ARG = 5 THEN D$ = "TAKING THE PICTURE REVEALS A FUSEBOX": PRINT D2$: ILOC(ARG) = 0: ILOC(IMMOFF+7) = 30: GOTO 2500
6590 ILOC(ARG) = 0: PRINT VOCAB$(INPTK(2)); ": TAKEN": GOTO 5000
6600 IF ILOC(ARG) <> 0 THEN D$= "YOU AREN'T CARRYING IT": PRINT D2$: GOTO 5000
6630 IC = IC - 1: IF LCL = 17 AND ARG = 10 AND REXIT(17,1) <= 0 THEN D$= "THE DOG LOOKS DISGUSTED.  MAYBE YOU SHOULD EAT IT.": PRINT D2$: GOTO 6690
6640 IF LCL = 17 AND ARG = 2 AND REXIT(17,1) <= 0 THEN D$= "THE DOG CHEWS HIS FAVORITE TOY AND IS SOON ASLEEP": PRINT D2$: ILOC(ARG) = -999: REXIT(17,1)=18: GOTO 2500
6650 IF LCL = 29 AND ARG = 12 AND REXIT(29,5) <= 0 THEN D$= "THE BOXSPRING COVERS THE GAP IN THE STAIRS": PRINT D2$: ILOC(ARG) = -999: REXIT(29,5) = 2: REXIT(2,6) = 29: GOTO 2500
6690 ILOC(ARG) = LCL: PRINT VOCAB$(INPTK(2)); ": DROPPED": GOTO 5000

6700 ARG = INPTK(2) - ITEMOFF
6740 IF ILOC(ARG) <> 0 AND ILOC(ARG) <> LCL THEN D$= "IT'S NOT HERE": PRINT D2$: GOTO 5000
...
6800 IF ILOC(7) <> 0 THEN D$ = "YOU DON'T HAVE A KEY!": PRINT D2$: GOTO 5000
6820 IF LCL = 5 THEN D$= "THE KEY DOESN'T FIT THE LOCK": PRINT D2$: GOTO 5000
6830 IF LCL = 17 THEN D$ = "YOU UNLOCK THE DOOR. BEWARE!": PRINT D2$: REXIT(17,4) = 20: GOTO 2500
6899 D$ = "HUH?": PRINT D2$: GOTO 5000
6900 IF ILOC(ARG) <> 0 THEN D$= "YOU DON'T HAVE IT!": PRINT D2$: GOTO 5000
6920 IF ARG <> 10 THEN D$= "YOU CAN'T EAT THAT!": PRINT D2$: GOTO 5000
6930 D$= "THERE WAS A DIAMOND HIDDEN INSIDE THE GAINESBURGER": PRINT D2$: ILOC(ARG) = -2: ILOC(17) = 0: GOTO 2500
6950 AIMM = ARG - IMMOFF
6960 IF AIMM >= 1 AND AIMM <= 4 THEN ON AIMM GOTO 6970, 6975, 6980
6965 D$= "YOU CAN'T DO THAT": PRINT D2$: GOTO 5000
6970 D$= "IT'S TOO HEAVY FOR YOU TO MOVE": PRINT D2$: GOTO 5000
6975 D$= "YOUR BACK IS ACTING UP": PRINT D2$: GOTO 5000
6980 D$= "THAT SEEMS POINTLESS AND UNSANITARY": PRINT D2$: GOTO 5000

7000 GOARG = INPTK(1): IF GOARG > 6 THEN GOARG = GOARG - 6
7010 IF REXIT(LCL, GOARG) > 0 THEN LCL = REXIT(LCL, GOARG): GOTO 2500
7020 IF LCL = 12 AND GOARG = 5 THEN D$= "YOU'RE AFRAID OF THE DARK": PRINT D2$: GOTO 5000
7025 IF LCL = 17 AND GOARG = 1 THEN D$= "YOU NEVER DID LIKE THAT DOG": PRINT D2$: GOTO 5000
7030 IF LCL = 23 AND REXIT(23, 6) <= 0 THEN D$ = "THE DUMBWAITER MECHANISM IS CORRODED AND WON'T MOVE": PRINT D2$: GOTO 5000
7099 D$= "YOU CAN'T GO THAT WAY": PRINT D2$: GOTO 5000
7100 IF COMM < 23 OR COMM > 30 THEN GOTO 6899
7120 ARG = INPTK(2) - ITEMOFF: IF COMM <> 27 AND ARG < 1 or ARG > LASTITEM THEN D$ = "HUH?": PRINT D2$: GOTO 5000
7130 IF COMM <> 23 AND COMM <> 29 AND ILOC(ARG) <> LCL AND ILOC(ARG) <> 0 THEN D$= "IT'S NOT HERE": PRINT D2$: GOTO 5000
7140 ON COMM-22 GOTO 6800, 6899, 6899, 7200, 7400, 7500, 7700, 7800
7200 IF ARG < IMMOFF THEN D$= "YOU CAN JUST TAKE THAT": PRINT D2$: GOTO 5000

7220 AIMM = ARG - IMMOFF: MVARG = INPTK(3) - ITEMOFF: IF AIMM < 1 OR AIMM > 3 THEN D$ = "YOU CAN'T DO THAT": PRINT D2$: GOTO 5000
7230 IF ILOC(MVARG) <> 0 THEN D$ = "YOU DON'T HAVE IT!": PRINT D2$: GOTO 5000
7240 ON AIMM GOTO 7250, 7300, 7350
7250 IF MVARG <> 4 OR ILOC(3) >= 0 THEN D$= "YOU CAN'T DO THAT": PRINT D2$: GOTO 5000
7260 D$= "YOU JACK UP THE FRIDGE AND FIND A FUSE UNDER IT": PRINT D2$: ILOC(3)=LCL: GOTO 2500
7300 IF MVARG <> 13 OR ILOC(2) >= 0 THEN D$ = "YOU CAN'T DO THAT": PRINT D2$: GOTO 5000
7310 D$="YOU MOVE THE COUCH AND FIND A TEDDYBEAR BEHIND IT": PRINT D2$: ILOC(2)=LCL: GOTO 2500
7350 IF MVARG <> 11 THEN D$ = "YOU CAN'T DO THAT": PRINT D2$: GOTO 5000
7360 D$="MOVING THE CLOTHES REVEALS A LAUNDRY CHUTE TO THE BASEMENT": PRINT D2$: REXIT(LCL,6) = 27: GOTO 2500

7400 IF LCL <> 20 THEN D$ = "HUH?": PRINT D2$: GOTO 5000
7450 IF INPTK(3) - ITEMOFF <> IMMOFF + 4 THEN D$ = "HUH?": PRINT D2$: GOTO 5000
7460 DOORDIR = INPTK(2) - DIROFF: IF DOORDIR < 1 OR DOORDIR > 3 THEN D$ = "HUH?": PRINT D2$: GOTO 5000
7470 GOSUB 8025
7480 IF DOORDIR = SAFED THEN D$= "OPENING THE DOOR REVEALS A DUMBWAITER": PRINT D2$: REXIT(LCL, 4) = 23: GOTO 2500
7490 IF INT(RND(2)) > 1 THEN D$= "A SHOT RINGS OUT!  IT WAS WELL-AIMED TOO.": PRINT D2$: GOTO 19000
7495 D$= "AN IRONING BOARD SLAMS ONTO YOUR HEAD": PRINT D2$: GOTO 19000
7500 IF LCL <> 12 THEN D$ = "YOU CAN'T DO THAT": PRINT D2$: GOTO 5000
7520 IF INPTK(2) - ITEMOFF <> 6 THEN D$= "YOU CAN'T TIE THAT": PRINT D2$: GOTO 5000
7530 IF INPTK(3) - ITEMOFF <> (IMMOFF + 5) THEN D$= "YOU CAN'T TIE TO THAT": PRINT D2$: GOTO 5000
7540 IF ILOC(6) <> 0 THEN D$ = "YOU DON'T HAVE IT!": PRINT D2$: GOTO 5000
7550 D$= "TIED": PRINT D2$: ILOC(6) = -12: IC = IC - 1: GOTO 2500
7600 IF ILOC(8) <> 0 THEN D$ = "HUH?": PRINT D2$: GOTO 5000

7610 IF LCL = 18 THEN D$= "THERE IS A FLASH OF LIGHT AND A CRACKING SOUND.  AN OPENING APPEARS IN THE EAST WALL": PRINT D2$: REXIT(18, 3) = 19: GOTO 2500
7620 INVERSE: D$= "WHEE!": PRINT D2$: NORMAL: PRINT "": GOTO 5000
7700 IF LCL <> 20 THEN D$ = "YOU CAN'T DO THAT": PRINT D2$: GOTO 5000
7705 IF ILOC(15) <> 0 THEN D$= "YOU DON'T HAVE ANY OIL": PRINT D2$: GOTO 5000
7720 IF INPTK(2) - ITEMOFF <> IMMOFF + 6 THEN D$ = "HUH?": PRINT D2$: GOTO 5000
7725 D$= "THE DUMBWAITER MECHANISM NOW RUNS SMOOTHLY": PRINT D2$: REXIT(23,6) = 24: GOTO 5000
7800 IF LCL <> 30 OR (INPTK(2) - ITEMOFF <> 3) THEN D$ = "YOU CAN'T DO THAT": D2$ = D$: GOTO 5000

7830 IF INPTK(3) - ITEMOFF <> (IMMOFF + 7) THEN D$= "YOU CAN'T PUT IT THERE": PRINT D2$: GOTO 5000
7840 IF ILOC(3) <> 0 THEN D$ = "YOU DON'T HAVE IT!": PRINT D2$: GOTO 5000

7850 D$= "YOU PUT THE FUSE IN THE BOX": PRINT D2$: ILOC(3) = -999: IC = IC - 1: REXIT(12,5) = 25: GOTO 5000
...
8080 D$ = "EXPERIMENTS ON ": D2$ = D$: NTMSG$ = D2$ + N1$
8085 D$ = " AND ": D2$ = D$: NTMSG$ = NTMSG$ + D2$ + N2$
8090 D$ = " DOORS PROCEEDING WELL; FILE FOR PATENT": D2$ = D$: NTMSG$=NTMSG$+D2$
8099 PRINT NTMSG$: RETURN

8200 IF LCL = 20 AND (INPTK(2) - ITEMOFF) = IMMOFF + 4 THEN D$= "PLEASE SPECIFY LEFT, CENTER, OR RIGHT": PRINT D2$: GOTO 5000
8299 D$ = "HUH?": PRINT D2$: GOTO 5000
...
10999 REM *** DECODE AND PRINT STRING ***
11000 GOSUB 10000: PRINT D2$: RETURN


14999 REM --- INTRO ---
15000 D$ = "TAYS HOUSE ADVENTURE": PRINT D2$
15010 D$ = "FIND TREASURES AND VALUABLES IN YOUR MAD UNCLE TAYS' HOUSE": PRINT D2$
15020 D$ = "TYPE SIMPLE COMMANDS: NORTH, SOUTH, ETC. TO MOVE (OR JUST 'N', 'S').": PRINT D2$
15030 D$ = "TAKE AND DROP, INVENTORY, LOOK, READ, MOVE, AND SO ON.": PRINT D2$
15040 D$ = "SOME COMMANDS ARE COMPLEX: 'MOVE THE HUBCAP WITH THE SPANNER'": PRINT D2$
15045 RETURN

19000 D$ = "YOU HAVE DIED": PRINT D2$: GOTO 19999
19900 GOSUB 9500: IF SCORE = 100 THEN D$ = "YOU HAVE WON THE GAME!": PRINT D2$
19999 END
