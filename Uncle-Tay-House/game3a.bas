...
2049 REM *** DEBUG ***
2050 LCL=20: ILOC(7)=0: ILOC(9)=0: ILOC(15)=0

2499 REM *** LOOP PRINCIPAL ***

2500 IF LCL = 30 THEN D$= "... AND SPRING BACK": PRINT D$: LCL = 12
2525 IF LCL = 31 THEN GOTO 19900

2509 REM *** SHOW LOCAL ***

2510 PRINT " ": PRINT RNAME$(LCL): PRINT RDESC$(LCL)
...
2569 NEXT
2570 IF LCL = 17 AND REXIT(17,1) > 0 THEN D$="YOUR UNCLE'S DOBERMAN IS SNORING PEACEFULLY": PRINT D$
2580 IF LCL = 3 AND ILOC(6) = -12 THEN D$="A BUNGEE CORD DANGLES FROM THE RAILING ABOVE": PRINT D$
2582 IF LCL = 12 AND ILOC(6) = -12 THEN D$="A BUNGEE CORD DANGLES FROM THE RAILING": PRINT D$
2600 FOR I = 1 TO LASTITEM: IF ILOC(I) = LCL THEN PRINT "THERE IS A "; VOCAB$(I + ITEMOFF); " HERE"
2619 NEXT
2620 IF LCL = 2 AND ILOC(3) = -1 THEN D$= "SOMETHING IS BARELY VISIBLE UNDER THE FRIDGE": PRINT D$
2650 IF LCL = 3 AND ILOC(5) = 30 THEN D$= "THERE IS A PICTURE HIGH UP ON THE WALL": PRINT D$
4979 REM --- DEAL WITH MID-AIR
4980 IF LCL = 30 THEN TURN1 = 1

4999 REM *** READ INPUT ***

5000 IF TURN1 <> 1 AND LCL = 30 THEN D$= "...AND SPRING BACK": PRINT D$: LCL = 12: GOTO 2500
...
6299 REM -- JUMP --
6300 IF LCL <> 12 THEN D$= "WHO ARE YOU, DAVID LEE ROTH?": PRINT D$: GOTO 5000
6320 IF ILOC(6) <> -12 THEN D$ = "YOU FORGOT YOUR PARACHUTE": PRINT D$: GOTO 5000
6330 D$= "YOU BUNGEE OFF THE BALCONY...": PRINT D$: LCL = 30: GOTO 2510
...
6510 IF ILOC(ARG) = 30 AND LCL = 3 AND ARG = 5 THEN D$= "IT'S TOO HIGH": PRINT D$: GOTO 5000
6520 IF ILOC(ARG) <> LCL THEN D$ = "IT'S NOT HERE 1": PRINT D$: GOTO 5000

6530 IF IC >= 8 THEN D$ = "YOU'RE CARRYING TOO MUCH": PRINT D$: GOTO 5000
6540 IF ARG > IMMOFF THEN D$ = "IT'S TOO HEAVY": PRINT D$: GOTO 5000
6559 IF LCL = 29 AND ARG = 12 THEN D$ = "YOU CAN'T DO THAT 2": PRINT D$: GOTO 5000
6570 IC = IC + 1: IF LCL = 30 AND ARG = 5 THEN D$ = "TAKING THE PICTURE REVEALS A FUSEBOX": PRINT D$: ILOC(ARG) = 0: ILOC(IMMOFF+7) = 30: GOTO 2500
6590 ILOC(ARG) = 0: PRINT VOCAB$(INPTK(2)); ": TAKEN": GOTO 5000

6600 IF ILOC(ARG) <> 0 THEN D$= "YOU AREN'T CARRYING IT": PRINT D$: GOTO 5000
6630 IC = IC - 1: IF LCL = 17 AND ARG = 10 AND REXIT(17,1) <= 0 THEN D$= "THE DOG LOOKS DISGUSTED. MAYBE YOU SHOULD EAT IT.": PRINT D$: GOTO 6690
6640 IF LCL = 17 AND ARG = 2 AND REXIT(17,1) <= 0 THEN D$= "THE DOG CHEWS HIS FAVORITE TOY AND IS SOON ASLEEP": PRINT D$: ILOC(ARG) = -999: REXIT(17,1)=18: GOTO 2500
6650 IF LCL = 29 AND ARG = 12 AND REXIT(29,5) <= 0 THEN D$= "THE BOXSPRING COVERS THE GAP IN THE STAIRS": PRINT D$: ILOC(ARG) = -999: REXIT(29,5) = 2: REXIT(2,6) = 29: GOTO 2500
6690 ILOC(ARG) = LCL: PRINT VOCAB$(INPTK(2)); ": DROPPED": GOTO 5000

6700 ARG = INPTK(2) - ITEMOFF
6740 IF ILOC(ARG) <> 0 AND ILOC(ARG) <> LCL THEN D$= "IT'S NOT HERE 2": PRINT D$: GOTO 5000
...
6820 IF LCL = 5 THEN D$= "THE KEY DOESN'T FIT THE LOCK": PRINT D$: GOTO 5000
6830 IF LCL = 17 THEN D$ = "YOU UNLOCK THE DOOR. BEWARE!": PRINT D$: REXIT(17,4) = 20: GOTO 2500
...
7020 IF LCL = 12 AND GOARG = 5 THEN D$= "YOU'RE AFRAID OF THE DARK": PRINT D$: GOTO 5000
7025 IF LCL = 17 AND GOARG = 1 THEN D$= "YOU NEVER DID LIKE THAT DOG": PRINT D$: GOTO 5000
7030 IF LCL = 23 AND REXIT(23, 6) <= 0 THEN D$ = "THE DUMBWAITER MECHANISM IS CORRODED AND WON'T MOVE": PRINT D$: GOTO 5000
...
7129 REM *** 23-UNLOCK 29-OIL 28-OILCAN *** AND ILOC(ARG) <> LCL AND ILOC(ARG) <> 0
...
7260 D$= "YOU JACK UP THE FRIDGE AND FIND A FUSE UNDER IT": PRINT D$: ILOC(3)=LCL: GOTO 2500

7300 IF MVARG <> 13 OR ILOC(2) >= 0 THEN D$ = "YOU CAN'T DO THAT 6": PRINT D$: GOTO 5000
7310 D$="YOU MOVE THE COUCH AND FIND A TEDDYBEAR BEHIND IT": PRINT D$: ILOC(2)=LCL: GOTO 2500

7350 IF MVARG <> 11 THEN D$ = "YOU CAN'T DO THAT 7": PRINT D$: GOTO 5000
7360 D$="MOVING THE CLOTHES REVEALS A LAUNDRY CHUTE TO THE BASEMENT": PRINT D$: REXIT(LCL,6) = 27: GOTO 2500

7400 IF LCL <> 20 THEN D$ = "HUH? 6": PRINT D$: GOTO 5000
...
7470 GOSUB 8025
7480 IF DOORDIR = SAFED THEN D$= "OPENING THE DOOR REVEALS A DUMBWAITER": PRINT D$: REXIT(LCL, 4) = 23: GOTO 2500
...
7500 IF LCL <> 12 THEN D$ = "YOU CAN'T DO THAT 8": PRINT D$: GOTO 5000
...
7610 IF LCL = 18 THEN D$= "THERE IS A FLASH OF LIGHT AND A CRACKING SOUND. AN OPENING APPEARS IN THE EAST WALL": PRINT D$: REXIT(18, 3) = 19: GOTO 2500
7620 D$= "WHEE!": PRINT D$: PRINT "": GOTO 5000

7700 IF LCL <> 20 THEN D$ = "YOU CAN'T DO THAT 9": PRINT D$: GOTO 5000
...
7800 IF LCL <> 30 OR (INPTK(2) - ITEMOFF <> 3) THEN D$ = "YOU CAN'T DO THAT 10": PRINT D$: GOTO 5000
...
8200 IF LCL = 20 AND (INPTK(2) - ITEMOFF) = IMMOFF + 4 THEN D$= "PLEASE SPECIFY LEFT, CENTER, OR RIGHT": PRINT D$: GOTO 5000
