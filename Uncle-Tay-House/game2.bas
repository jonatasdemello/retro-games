...
420 DATA "RAILING", "DUMBWAITER", "FUSEBOX", "THE", "TO", "WITH", "USING"
...
467 DATA "PRE-IPO SHARES OF APOLLO COMPUTING HAVE TO BE WORTH ... SOMETHING5"
...
700 DIROFF = 30: ITEMOFF = 33: LASTITEM = 27: IMMOFF = 20: IMOFF = 20 : LWRD = 60: NXDESC = 7
...
2049 REM *** DEBUG ***
2050 LOC=20: ILOC(7)=0: ILOC(9)=0: ILOC(15)=0

2500 IF LOC = 30 THEN D$= "... AND SPRING BACK": PRINT D$: LOC = 12
...
2569 NEXT
2570 IF LOC = 17 AND REXIT(17,1) > 0 THEN D$="YOUR UNCLE'S DOBERMAN IS SNORING PEACEFULLY": PRINT D$
2580 IF LOC = 3 AND ILOC(6) = -12 THEN D$="A BUNGEE CORD DANGLES FROM THE RAILING ABOVE": PRINT D$
2582 IF LOC = 12 AND ILOC(6) = -12 THEN D$="A BUNGEE CORD DANGLES FROM THE RAILING": PRINT D$
2600 FOR I = 1 TO LASTITEM: IF ILOC(I) = LOC THEN PRINT "THERE IS A "; VOCAB$(I + ITEMOFF); " HERE"
2619 NEXT
2620 IF LOC = 2 AND ILOC(3) = -1 THEN D$= "SOMETHING IS BARELY VISIBLE UNDER THE FRIDGE": PRINT D$
2650 IF LOC = 3 AND ILOC(5) = 30 THEN D$= "THERE IS A PICTURE HIGH UP ON THE WALL": PRINT D$
4999 IF LOC = 30 THEN TURN1 = 1
5000 IF TURN1 <> 1 AND LOC = 30 THEN D$= "...AND SPRING BACK": PRINT D$: LOC = 12: GOTO 2500
...
6300 IF LOC <> 12 THEN D$= "WHO ARE YOU, DAVID LEE ROTH?": PRINT D$: GOTO 5000
6320 IF ILOC(6) <> -12 THEN D$ = "YOU FORGOT YOUR PARACHUTE": PRINT D$: GOTO 5000
6330 D$= "YOU BUNGEE OFF THE BALCONY...": PRINT D$: LOC = 30: GOTO 2510
...
6510 IF ILOC(ARG) = 30 AND LOC = 3 AND ARG = 5 THEN D$= "IT'S TOO HIGH": PRINT D$: GOTO 5000
6520 IF ILOC(ARG) <> LOC THEN D$ = "IT'S NOT HERE": PRINT D$: GOTO 5000

6530 IF IC >= 8 THEN D$ = "YOU'RE CARRYING TOO MUCH": PRINT D$: GOTO 5000
6540 IF ARG > IMMOFF THEN D$ = "IT'S TOO HEAVY": PRINT D$: GOTO 5000
6559 IF LOC = 29 AND ARG = 12 THEN D$ = "YOU CAN'T DO THAT": PRINT D$: GOTO 5000
6570 IC = IC + 1: IF LOC = 30 AND ARG = 5 THEN D$ = "TAKING THE PICTURE REVEALS A FUSEBOX": PRINT D$: ILOC(ARG) = 0: ILOC(IMMOFF+7) = 30: GOTO 2500
6590 ILOC(ARG) = 0: PRINT VOCAB$(INPTK(2)); ": TAKEN": GOTO 5000
6600 IF ILOC(ARG) <> 0 THEN D$= "YOU AREN'T CARRYING IT": PRINT D$: GOTO 5000
6630 IC = IC - 1: IF LOC = 17 AND ARG = 10 AND REXIT(17,1) <= 0 THEN D$= "THE DOG LOOKS DISGUSTED.  MAYBE YOU SHOULD EAT IT.": PRINT D$: GOTO 6690
6640 IF LOC = 17 AND ARG = 2 AND REXIT(17,1) <= 0 THEN D$= "THE DOG CHEWS HIS FAVORITE TOY AND IS SOON ASLEEP": PRINT D$: ILOC(ARG) = -999: REXIT(17,1)=18: GOTO 2500
6650 IF LOC = 29 AND ARG = 12 AND REXIT(29,5) <= 0 THEN D$= "THE BOXSPRING COVERS THE GAP IN THE STAIRS": PRINT D$: ILOC(ARG) = -999: REXIT(29,5) = 2: REXIT(2,6) = 29: GOTO 2500
6690 ILOC(ARG) = LOC: PRINT VOCAB$(INPTK(2)); ": DROPPED": GOTO 5000
6700 ARG = INPTK(2) - ITEMOFF
6740 IF ILOC(ARG) <> 0 AND ILOC(ARG) <> LOC THEN D$= "IT'S NOT HERE": PRINT D$: GOTO 5000
...
6820 IF LOC = 5 THEN D$= "THE KEY DOESN'T FIT THE LOCK": PRINT D$: GOTO 5000
6830 IF LOC = 17 THEN D$ = "YOU UNLOCK THE DOOR. BEWARE!": PRINT D$: REXIT(17,4) = 20: GOTO 2500
...
7020 IF LOC = 12 AND GOARG = 5 THEN D$= "YOU'RE AFRAID OF THE DARK": PRINT D$: GOTO 5000
7025 IF LOC = 17 AND GOARG = 1 THEN D$= "YOU NEVER DID LIKE THAT DOG": PRINT D$: GOTO 5000
7030 IF LOC = 23 AND REXIT(23, 6) <= 0 THEN D$ = "THE DUMBWAITER MECHANISM IS CORRODED AND WON'T MOVE": PRINT D$: GOTO 5000
...
7130 IF COMM <> 23 AND COMM <> 29 AND ILOC(ARG) <> LOC AND ILOC(ARG) <> 0 THEN D$= "IT'S NOT HERE": PRINT D$: GOTO 5000
...
7260 D$= "YOU JACK UP THE FRIDGE AND FIND A FUSE UNDER IT": PRINT D$: ILOC(3)=LOC: GOTO 2500
7300 IF MVARG <> 13 OR ILOC(2) >= 0 THEN D$ = "YOU CAN'T DO THAT": PRINT D$: GOTO 5000
7310 D$="YOU MOVE THE COUCH AND FIND A TEDDYBEAR BEHIND IT": PRINT D$: ILOC(2)=LOC: GOTO 2500
7350 IF MVARG <> 11 THEN D$ = "YOU CAN'T DO THAT": PRINT D$: GOTO 5000
7360 D$="MOVING THE CLOTHES REVEALS A LAUNDRY CHUTE TO THE BASEMENT": PRINT D$: REXIT(LOC,6) = 27: GOTO 2500
7400 IF LOC <> 20 THEN D$ = "HUH?": PRINT D$: GOTO 5000
...
7470 GOSUB 8025
7480 IF DOORDIR = SAFED THEN D$= "OPENING THE DOOR REVEALS A DUMBWAITER": PRINT D$: REXIT(LOC, 4) = 23: GOTO 2500
...
7500 IF LOC <> 12 THEN D$ = "YOU CAN'T DO THAT": PRINT D$: GOTO 5000
...
7610 IF LOC = 18 THEN D$= "THERE IS A FLASH OF LIGHT AND A CRACKING SOUND.  AN OPENING APPEARS IN THE EAST WALL": PRINT D$: REXIT(18, 3) = 19: GOTO 2500
7620 INVERSE: D$= "WHEE!": PRINT D$: NORMAL: PRINT "": GOTO 5000
7700 IF LOC <> 20 THEN D$ = "YOU CAN'T DO THAT": PRINT D$: GOTO 5000
...
7800 IF LOC <> 30 OR (INPTK(2) - ITEMOFF <> 3) THEN D$ = "YOU CAN'T DO THAT": PRINT D$: GOTO 5000
...
8200 IF LOC = 20 AND (INPTK(2) - ITEMOFF) = IMOFF + 4 THEN D$= "PLEASE SPECIFY LEFT, CENTER, OR RIGHT": PRINT D$: GOTO 5000
